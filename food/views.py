...
def dinner(request):
    ...
    products = Product.objects.all()
    context = {'products': products,'cartItems': cartItems}
    return render(request, 'food/dinner.html', context)
...
def forgotPass(request):
    # context= {'form':form}
    # return render(request, 'food/Forgot_Pass.html',context)
    return render(request, 'food/Forgot_Pass.html')


from django.shortcuts import render, redirect        # ANCHOR
from django.core.mail import send_mail, BadHeaderError        # ANCHOR
from django.http import HttpResponse        # ANCHOR
from django.contrib.auth.forms import PasswordResetForm        # ANCHOR
from django.contrib.auth.models import User        # ANCHOR
from django.template.loader import render_to_string        # ANCHOR
from django.db.models.query_utils import Q        # ANCHOR
from django.utils.http import urlsafe_base64_encode        # ANCHOR
from django.contrib.auth.tokens import default_token_generator        # ANCHOR
from django.utils.encoding import force_bytes        # ANCHOR


def password_reset_request(request):        # ANCHOR
	if request.method == "POST":
			...
			if associated_users.exists():
				for user in associated_users:
					...
					try:
						...
						# send_mail(subject, email, '2016BIT042@sggs.ac.in ' , [user.email], fail_silently=False)
...
def dashboard(request):
    ...
    hd = orders.filter(home_delivery='Yes').order_by('-date_ordered')

    context = {'orders': orders, 'customers': customers,'orderitems':orderitems,'products':products,'payment':payment,
               'total_customers': total_customers, 'total_orders': total_orders, 'delivered': delivered, 'pending': pending,'hd':hd}
...
def customer(request,pk_test):
    ...
    orders_cust_count = orders.filter(customer=customer).count()
    ...
    context = {'customer': customer, 'orders': orders, 'orderitems':orderitems,
               'orders_count': orders_count, 'myFilter': myFilter,'orders_cust_count':orders_cust_count}
